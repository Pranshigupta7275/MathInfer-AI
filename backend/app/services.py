...
async def transcribe_audio_huggingface(audio_data: bytes) -> str:
    """Transcribes audio data using the Hugging Face Inference API for Whisper."""
    if not config.HUGGINGFACE_API_TOKEN or not config.HUGGINGFACE_WHISPER_ENDPOINT:
        logger.warning("Hugging Face API token or endpoint not configured. Transcription disabled.")
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(
                config.HUGGINGFACE_WHISPER_ENDPOINT,
                headers=headers,
                content=audio_data # Send raw audio bytes
            )
            response.raise_for_status() # Raise exception for 4xx/5xx errors

        result = response.json()
        transcription = result.get("text", "")
        logger.info("Audio transcribed successfully via Hugging Face.")
        return transcription

    except httpx.RequestError as e:
        logger.error(f"Error requesting Hugging Face transcription: {e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                            detail=f"Error communicating with transcription service: {e}")
    except httpx.HTTPStatusError as e:
        logger.error(f"Hugging Face transcription failed with status {e.response.status_code}: {e.response.text}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                            detail=f"Transcription service returned an error: {e.response.status_code}")
    except Exception as e:
        logger.error(f"Unexpected error during Hugging Face transcription: {e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
# ... (other service functions like transcribe_audio_huggingface, refresh_gemini_file_if_needed can remain if still used and compatible)
...
# Ensure transcribe_audio_huggingface is present if speech_router.py depends on it.
# async def transcribe_audio_huggingface(audio_data: bytes) -> str: ...
